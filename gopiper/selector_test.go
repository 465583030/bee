...
func TestJsonPipe(t *testing.T) {
	req := goreq.New()
	_, body, _ := req.Get("http://s.m.taobao.com/search?&q=qq&atype=b&searchfrom=1&from=1&sst=1&m=api4h5").End()
	...
	fmt.Println(pipe.PipeBytes([]byte(body), "json"))
}
...
func TestHtmlPipe(t *testing.T) {
	...
	req := goreq.New()
	_, body, _ := req.Get("http://movie.douban.com/subject/25850640/").EndBytes()
	...
	resp2, body2, _ := req.Get("http://movie.douban.com/subject/2035218/?from=tag_all").EndBytes()
...
func TestHtmlJingJiang(t *testing.T) {
	req := goreq.New()
	_, body, _ := req.Get("http://www.jjwxc.net/bookbase_slave.php?submit=&booktype=&opt=&page=3&endstr=&orderstr=4").EndBytes()

	body_utf8 := mahonia.NewDecoder("gb18030").ConvertString(string(body))
	body = []byte(body_utf8)
...
func TestBaiduLocal(t *testing.T) {
	req := goreq.New()
	_, body, _ := req.Get("http://www.baidu.com/s?wd=采集关键词&rn=50&tn=baidulocal").EndBytes()
